...
class MultiPIE():
    ...
    """
    def __init__(self, dir, keypoint_dir, datasplit='train', Random=True, LOAD_60_LABEL=False, MIRROR_TO_ONE_SIDE=True, RANDOM_VERIFY=False,
                 GENERATE_MASK=False, source='without90', testing = False):
        self.dir = dir 
        
        self.feat5ptDir = keypoint_dir        
    ...
    def load_image(self, filename):
            ...
            """
            im = Image.open(os.path.join(self.dir, filename))
            ...
            if self.MIRROR_TO_ONE_SIDE and self.findPose(filename) < 0:
                in_ = in_[:,::-1,:]
            feature_raw = [features[i].tostring() for i in range(1,5)]
            #import ipdb; ipdb.set_trace()
            return tuple([in_.tostring()] + feature_raw)

    def load_label_mask(self, filename, labelnum=-1):

        _, labelname = self.findSameIllumCodeLabelpath(filename)
        im = Image.open(os.path.join(self.dir, labelname))
        ...
        if not self.LOAD_60_LABEL:
            if self.findPose(filename) < 0:#random.random() < 0.5:
                ...
                feats = [feats[0], feats[2], feats[1], feats[3], feats[4]]
        #print("not flipping!")
        feature_raw = [feats[i].tostring() for i in range(1,5)]
        assert label.size ==  IMAGE_SIZE * IMAGE_SIZE * 3
        assert len(feature_raw) == 4

        return tuple([label.tostring()] + feature_raw)
    ...
    def GetFeatureParts(self, img_resize, filename, label=False):
        ...
        if True:#not label:
            featpath = os.path.join(self.feat5ptDir, filename[0:-15] + '_trans.5pt')
            #print(filename)
        ...
        #print(trans_points)
        eyel_crop = np.zeros([EYE_H,EYE_W,3], dtype=dtype);
        ...
        eyer_crop = np.zeros([EYE_H,EYE_W,3], dtype=dtype);
        ...
        month_crop = np.zeros([MOUTH_H,MOUTH_W,3], dtype=dtype);
        ...
        nose_crop = np.zeros([NOSE_H,NOSE_W,3], dtype=dtype);
...
def get_multiPIE_batches(data_dir,
                         batch_size,
                         num_batches=1,
                         label_offset=0,
                         cycle_length=1,
    ...
    FLAGS = config_factory.ConfigFactory().flags;
    ...
    def _extract_image_and_label(record):
        ...
        features = tf.parse_single_example(
            ...
            features={
                ...
                'eyel': tf.FixedLenFeature([], tf.string),
                'eyer': tf.FixedLenFeature([], tf.string),
                'nose': tf.FixedLenFeature([], tf.string),
                'mouth': tf.FixedLenFeature([], tf.string),
                'label': tf.FixedLenFeature([], tf.string),
                'leyel': tf.FixedLenFeature([], tf.string),
                'leyer': tf.FixedLenFeature([], tf.string),
                'lnose': tf.FixedLenFeature([], tf.string),
                'lmouth': tf.FixedLenFeature([], tf.string),    
            })

        def decode_and_cast_feature(feature, height, width):        
            image_raw = tf.decode_raw(feature, tf.uint8)
            image_raw.set_shape(height * width * 3)
            image_raw = tf.reshape(image_raw, [1, height, width, 3])
            image = tf.image.resize_bilinear(image_raw, [height, width])
            #image.set_shape(image_size * image_size * 3)
            image = tf.reshape(image, [height, width, 3])
            image = tf.cast(image, tf.float32) / 255.
            return image

        return  (decode_and_cast_feature(features['image'], IMAGE_SIZE, IMAGE_SIZE),
                decode_and_cast_feature(features['eyel'], EYE_H, EYE_W),
                decode_and_cast_feature(features['eyer'], EYE_H, EYE_W),
                decode_and_cast_feature(features['nose'], NOSE_H, NOSE_W),
                decode_and_cast_feature(features['mouth'], MOUTH_H, MOUTH_W),
                decode_and_cast_feature(features['label'], IMAGE_SIZE, IMAGE_SIZE),
                decode_and_cast_feature(features['leyel'], EYE_H, EYE_W),
                decode_and_cast_feature(features['leyer'], EYE_H, EYE_W),
                decode_and_cast_feature(features['lnose'], NOSE_H, NOSE_W),
                decode_and_cast_feature(features['lmouth'], MOUTH_H, MOUTH_W))

    dataset = dataset.map(_extract_image_and_label, num_parallel_calls=16)
    dataset = dataset.prefetch(batch_size * num_batches)
